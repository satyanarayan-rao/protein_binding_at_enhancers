...
for job_k in job_id_pkl:  
    input_fp = open(job_id_pkl[job_k][0])
    #footprint_dict = pickle.load(open(sys.argv[2], "rb"))
    lflank = int(job_id_pkl[job_k][2])
    rflank = int(job_id_pkl[job_k][3])
    lextend = int(job_id_pkl[job_k][4])
    rextend = int(job_id_pkl[job_k][5])
    
    out_fp = open(job_id_pkl[job_k][6], "w")
    strand = job_id_pkl[job_k][7]
    out_verb_fp = open(job_id_pkl[job_k][8], "w")
