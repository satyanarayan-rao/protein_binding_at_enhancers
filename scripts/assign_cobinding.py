...
cobinding_verbose_fp = open(sys.argv[9])
mvec_out_fp = open(sys.argv[10], "w")
mvec_out_150 = open(sys.argv[11], "w")
...
for line in inp_fp:
    ...
    if (len(primary_percentage_orange_per_footprint) > 0 ) and\
       (len(secondary_percentage_orange_per_footprint) > 0): 
        ...
        #print (["here", abs_start_of_max_orange_read_primary, abs_start_of_max_orange_read_secondary, length_of_max_orange_read_primary, length_of_max_orange_read_secondary, read_start_abs, read_end_abs])
        if (max_orange_primary > 30) and (max_orange_secondary > 30):
                ...
                if (abs_start_of_max_orange_read_primary != read_start_abs) and\
                   (abs_start_of_max_orange_read_primary + length_of_max_orange_read_primary - 1!= read_end_abs):
                    if length_of_max_orange_read_primary <=100:
                        ...
                        primary_assignment_dict[read_id]["binding_label"] = "2" # TF
                        secondary_assignment_dict[read_id]["binding_label"] = "2" # TF 
                elif length_of_max_orange_read_primary <=100:
                     primary_assignment_dict[read_id]["binding_label"] = "1" # TF
                     secondary_assignment_dict[read_id]["binding_label"] = "1" # TF 
                else:
                     primary_assignment_dict[read_id]["binding_label"] = "2" # TF
            ...
            else: # two footprints are different handle them separately
                if (abs_start_of_max_orange_read_primary != read_start_abs) and\
                   (abs_start_of_max_orange_read_primary + length_of_max_orange_read_primary - 1!= read_end_abs): 
                    if length_of_max_orange_read_primary <=50:
                        primary_assignment_dict[read_id]["binding_label"] =  "1" 
                    else:
                        primary_assignment_dict[read_id]["binding_label"] =  "2"
                else: 
                    if length_of_max_orange_read_primary >50: 
                        primary_assignment_dict[read_id]["binding_label"] = "2"
                    else:
