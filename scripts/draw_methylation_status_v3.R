...
# args[11]: vertical dash line left
# args[12]: vertical dash line right
# args[13]: site id 
...
for (label in uniq_cluster_label){
    ...
    if(cnt == len_label){
        ...
        mtext(args[13], side = 1, cex = 0.75, line = 1.5, at = mid_x)
    } else{
    ...
    for (m_vec in sub_df$methylation_vec){
        ...
        for(m_s in kp){
            if (identical (m_s, ".")){
                ...
                draw.circle(x_tic, y_val + 0.25 , circle_point_size, border = "#bdbdbd", col = "#969696", lwd = 0.2)
            }else if (identical(m_s, tolower(m_s))){
                draw.circle(x_tic, y_val + 0.25 , circle_point_size, lwd = 0.2)
                non_orange_count = non_orange_count + 1
            }else{
                draw.circle(x_tic, y_val + 0.25 , circle_point_size, col="orange", border = "black", lwd = 0.2)
